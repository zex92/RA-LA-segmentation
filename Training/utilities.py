...
def train(model, data_in, loss, optim, max_epochs, model_dir, test_interval=1,
    ...
    for epoch in range(max_epochs):
        ...
        #if (epoch + 1) % test_interval == 0:
            #_validate(model,device,val_loader,loss)
...
"""def train(model, data_in, max_epochs, model_dir, test_interval=1,
          device="cuda" if torch.cuda.is_available() else "cpu"):


    train_loader, test_loader = data_in

    # Replace the train function with a PyTorch Lightning Trainer
    trainer = pl.Trainer(max_epochs=max_epochs, devices=1 if device == "cuda" else 0, log_every_n_steps=16,    enable_checkpointing=True,
...
def train_kfold(model, dataloader, loss, optim,device="cuda" if torch.cuda.is_available() else "cpu"):
    model.train()
    total_loss = 0
...
def validate_kfold(model, dataloader, criterion,device="cuda" if torch.cuda.is_available() else "cpu"):

    model.eval()  # evaluation mode
    loss_value = 0  # accumulate the losses here
    ...
    for val_data in dataloader:
        ...
        input, target = volume.to(device), label.to(device)  # send to device (GPU or CPU)
...
def calculate_weights(val1, val2):
    ...
    return torch.tensor(weights, dtype=torch.float32)
def k_fold_cross_validation(data_in, model, loss_fn, optimizer, k=3, epochs=50, device="cuda" if torch.cuda.is_available() else "cpu"):
    train_loader, test_loader, val_loader = data_in
    kfold = KFold(n_splits=k, shuffle=True, random_state=42)
    fold_results = []

    for fold, (train_indices, val_indices) in enumerate(kfold.split(data_in)):
        ...
        for epoch in range(epochs):
            train_loss = train_kfold(model,train_loader,loss_fn,optimizer)
            val_loss = validate_kfold(model, val_loader, loss_fn)
...
def _validate(model,device,validation_DataLoader,loss):
    # need to add validation data
    # need to need to turn the data into data loader (need to change some code in the preprossing)
